...
For local testing, the seed dataset contains pre-configured accounts for each role:

| Role | Login Identifier / Email | Password | Primary Capabilities |
| :--- | :--- | :--- | :--- |
| **👑 Administrator** | `admin@awt.org` | `admin123` | Full administrative control, intern approvals, certificate printing, analytics, settings |
| **🧑‍🏫 Supervisor** | `supervisor@awt.org` | `supervisor123` | Department intern tracking, task management, deliverable reviews, 10-point appraisal |
| **🎓 Legacy Intern** | `917` *(or any legacy ID e.g. `866`, `56`, `101`, or student email)* | `intern123` | Self-service dashboard, onboarding checklist, task submission, offer letter & certificate printing |

> ℹ️ **Auto-Provisioning**: Any of the 764 migrated historical interns can log in using their ID (`id`), ERP number (`ERPNO`), registered Email, or Cell Phone number with the default password `intern123`. A secure user session is provisioned immediately on first sign-in.

---

## 🌐 Production Deployment Guide

AWT-IMS is engineered for zero-downtime deployment on both Windows Server (IIS / Plesk) and Linux (Apache / Nginx):

### Windows Server / Plesk (IIS 10)
- The project includes a pre-configured `web.config` that handles:
  - Default document prioritization (`index.php`).
  - Request filtering to block direct browser access to `.sql`, `.accdb`, `.env`, `.ps1`, and `.log` files.
  - Hidden folder rules to secure `database/` and `.git/`.
  - HTTP security headers (`X-Content-Type-Options`, `X-Frame-Options`).

### Linux / cPanel (Apache 2.4+)
- The included `.htaccess` file provides:
  - `Options -Indexes` preventing directory browsing.
  - Direct denial of access to sensitive extensions via `<FilesMatch>`.
  - Rewrite rules preventing external web requests into `config/` or `database/`.
  - Standard security headers and PHP memory/upload parameter recommendations.

---

## 🛡️ Security Engineering

- **SQL Injection Defense**: Every single query interacting with user input utilizes PDO Prepared Statements with parameterized execution.
- **Cross-Site Scripting (XSS) Prevention**: Output sanitization using the custom `e()` helper (`htmlspecialchars(..., ENT_QUOTES, 'UTF-8')`) across all dashboard views and printable templates.
- **Cross-Site Request Forgery (CSRF) Protection**: Cryptographically secure anti-CSRF tokens (`bin2hex(random_bytes(32))`) embedded into every POST action (`apply.php`, intern forms, appraisals).
- **Session Hardening**: Cookie flags initialized with `HttpOnly=true`, `SameSite=Lax`, and `Secure` (auto-enabled on HTTPS).
- **Password Security**: Standard PHP `password_hash()` implementing BCRYPT with automatic background hashing upgrades upon successful authentication.
- **Upload Restrictions**: Strict MIME-type checking, file extension whitelisting, and directory access controls for uploaded documents and CVs.

---

## ⚠️ GitHub Upload & Security Notice

Before pushing this repository to a public GitHub account, follow these essential security precautions:

1. **Protect Database Passwords**:
   - Inspect `config/database.php` and verify that any live server production database passwords have been removed or replaced with environment variables (`getenv()`) or placeholders.
2. **Review Historical / Sensitive Data**:
   - The repository contains authentic legacy records from past academic years. If your institutional privacy policy requires anonymization of student emails or phone numbers, sanitize `database/awt_internship_full.sql` or add it to `.gitignore`.
3. **Exclude Binary Access Files**:
   - The root directory includes Microsoft Access files (`*.accdb`, ~8 MB total). Add `*.accdb` to `.gitignore` if you do not want to distribute binary Access databases publicly.
4. **Ensure `.gitignore` is Committed**:
